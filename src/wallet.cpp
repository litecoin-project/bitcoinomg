...
bool CWallet::AddWatchOnly(const CTxDestination &dest)
{
    if (!CCryptoKeyStore::AddWatchOnly(dest))
        return false;
    //nTimeFirstKey = 1; // No birthday information for watch-only keys. (and no birthday in 0.8.x)
    if (!fFileBacked)
        return true;
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, bool fOnlySpendable, const CCoinControl *coinControl) const
{
    vCoins.clear();

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (fOnlySpendable && mine != MINE_SPENDABLE) continue;
                if (!(pcoin->IsSpent(i)) && !IsLockedCoin((*it).first, i) && pcoin->vout[i].nValue > 0 &&
